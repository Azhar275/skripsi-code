...
class PolygonCoordinates(db.Model):
    ...
    def __init__(self,preference_num,x1,y1,x2,y2,x3,y3,x4,y4,cameraId,createdAt,updatedAt):
...
class Camera(db.Model):
    ...
    id=db.Column(db.Integer,primary_key=True,autoincrement=True)
    cameraName = db.Column(db.String(50), nullable=False)
    cameraUrl = db.Column(db.String(150), nullable=False)
    createdAt=db.Column(db.DateTime,nullable=False)

    def __init__(self, cameraName, cameraUrl, createdAt):
...
class cctvThread(threading.Thread):
    def __init__(self, id, limit):
        ...
        self.frameByte =  cv2.imencode('.jpg',cv2.imread("static/zone.png", cv2.IMREAD_COLOR))[1].tobytes()
        self.running = True
        self.daemon = True

    def run(self):
        tracker1 = Tracker()
        url = Camera.query.filter_by(id=self.id).first().cameraUrl
        streams = streamlink.streams(url)
        cap = cv2.VideoCapture(streams["480p"].url)
        poly_zone=getCoordinates(self.id)
        ...
        reloaded = 0  
        while self.running:
            print('Running Background '+str(self.id))
            failed = False
            success, self.frame, failed = timerImage(cap)
            if success == False:
                print('entered 2')
                ...
                self.frame=cv2.resize(self.frame,(880,440))
                time.sleep(5)
            ...
            if failed == True :
                ...
                self.frame=cv2.resize(self.frame,(880,440))
                results = model.predict(self.frame, classes=0, stream_buffer=True, stream=True, conf=0.5)
                ...
                for result in results:
                    ...
                    tracker1.update(self.frame, detections)
                    for track in tracker1.tracks:
                        ...
                        cv2.putText(self.frame, (str(track_id)),(x1,y1),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,0,255),2)
                        cv2.rectangle(self.frame, (x1, y1), (x2, y2), (colors[track_id % len(colors)]), 3)
                
                self.frame=cv2.polylines(self.frame,[np.array(polygon_zone,np.int32)],True,(0,0,0),4,cv2.LINE_AA)
                cv2.line(self.frame,(poly_zone['x1'], poly_zone['y1']),(poly_zone['x2'], poly_zone['y2']),(0,255,0),2)
                


                cv2.line(self.frame,(poly_zone['x3'], poly_zone['y3']),(poly_zone['x4'], poly_zone['y4']),(0,0,255),2)
                cv2.rectangle(self.frame, (0, 40), (200, 130), (255,255,255), -1)
                if len(enter_list)-len(out_list) >= self.limit :
                    cv2.putText(self.frame, ("Limit :"+str(self.limit)+" (PENUH)"),(10,60),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,255),2)
                    if not notified :
                        telegram_bot_sendtext("Ruangan penuh, segera tangani")
                        notified = True
                else :
                    cv2.putText(self.frame, ("Limit :"+str(self.limit)),(10,60),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                    notified = False
                cv2.putText(self.frame, ("Masuk :"+str(len(enter_list))),(10,80),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                cv2.putText(self.frame, ("Keluar :"+str(len(out_list))),(10,100),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                cv2.putText(self.frame, ("Kepadatan :"+str(len(enter_list)-len(out_list))),(10,120),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                buffer=cv2.imencode('.jpg',self.frame)[1] # change to index 1 to get the buffer
                self.frameByte=buffer.tobytes()# converting the image to bytes
...
def background_task():
    ...
    while True:
        ...
        print("Background task running with Celery... "+name)
        time.sleep(10)
...
def getCoordinates(id):
    ...
    with app.app_context(): # create a context for the database access
        ...
        # Using ORM Approach
        result=PolygonCoordinates.query.filter_by(cameraId=id).first()
        if result:
            ...
            newPolyCoordinates = PolygonCoordinates(1,200,300,500,300,500,100,200,100,id,func.now(),func.now())
            db.session.add(newPolyCoordinates)
            db.session.commit()
            print("No data found, inserting default data...")
            poly_coordinates['x1'] = 200
            poly_coordinates['y1'] = 300
            poly_coordinates['x2'] = 500
            poly_coordinates['y2'] = 300
            poly_coordinates['x3'] = 500
            poly_coordinates['y3'] = 100
            poly_coordinates['x4'] = 200
            poly_coordinates['y4'] = 100
...
def submitCoordinates():
    ...
    all_camera = session.get('all_camera')
    ...
    with app.app_context(): # create a context for the database access
        PolygonCoordinates.query.filter_by(cameraId=cam).update(updated_coordinates)
        db.session.commit()
    if all_camera :
        threading_list[cam].stop()
        threading_list[cam] = cctvThread(cam, limit)
        threading_list[cam].start()

    return redirect(url_for('settings', id=cam))

@app.route('/submit_limit',methods=['POST'])
def submitLimit():
    nLimit=request.form.get('limit')
    print ("received coordinates -> ",nLimit)
    cam = session.get('camera')
    cameraList = Camera.query.all()
    all_camera = session.get('all_camera')
    session['limit'] = int(nLimit)
    if all_camera:
        stop_all_thread()
        for i in cameraList:
            threading_list[i.id] = cctvThread(i.id, int(nLimit))
            threading_list[i.id].start()
    return redirect(url_for('settings', id=cam))
...
def run_all_thread():
    for i in threading_list.values():
       i.start()

def stop_all_thread():
    for i in list(threading_list.keys()) :
       threading_list[i].stop()
       threading_list.pop(i)
...
def annotatedStream(cam, limit):
    tracker1 = Tracker()
    url = Camera.query.filter_by(id=cam).first().cameraUrl
    # url = 'https://www.youtube.com/watch?v=DjdUEyjx8GM&pp=ygURbGl2ZSBqYXBhbiBzdHJlZXQ%3D'  # replace with your YouTube live stream URL
    streams = streamlink.streams(url)
    cap = cv2.VideoCapture(streams["480p"].url)


    # cap = cv2.VideoCapture("rtsp://admin:TCHIOT@192.168.1.4:554/Streaming/Channels/101") #indoor office cctv
    # cap = cv2.VideoCapture("rtsp://admin:admin123@192.168.25.254:554/Streaming/Channels/201")
    # cap = cv2.VideoCapture(0)  
    # temporarily stored here
    poly_zone=getCoordinates(cam)
    polygon_zone=np.array([[poly_zone['x1'],poly_zone['y1']],
                        [poly_zone['x2'],poly_zone['y2']],
                        [poly_zone['x3'],poly_zone['y3']],
                        [poly_zone['x4'],poly_zone['y4']]],
    ...
    while True:
        ...
        if failed == True :
            ...
            # converting the collected frame to image
            frame=cv2.resize(frame,(880,440))
            results = model.predict(frame, classes=0, stream_buffer=True, stream=True, conf=0.5)
            ...
            if len(enter_list)-len(out_list) >= limit :
                cv2.putText(frame, ("Limit :"+str(limit)+" (PENUH)"),(10,60),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,255),2)
                if not notified :
                    telegram_bot_sendtext("Ruangan penuh, segera tangani")
                    notified = True
            else :
                cv2.putText(frame, ("Limit :"+str(limit)),(10,60),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                notified = False
...
def backgroundAnnotatedStream(cam, limit):
    tracker1 = Tracker()
    url = Camera.query.filter_by(id=cam).first().cameraUrl
    # url = 'https://www.youtube.com/watch?v=DjdUEyjx8GM&pp=ygURbGl2ZSBqYXBhbiBzdHJlZXQ%3D'  # replace with your YouTube live stream URL
    streams = streamlink.streams(url)
...
def login():
#    threading.Thread(target=annotatedStream2, daemon=True).start()
   if session.get('login'):      
      return redirect(url_for('analytics', id=1))
   form = Login()
      ...
      if user:
         if bcrypt.check_password_hash(user.password, form.password.data):
            ...
            for i in camera_list:
                threading_list[i.id] = cctvThread(i.id, 5)
            return redirect(url_for('analytics', id=1))
...
def camera():
    ...
    return render_template('camera.html', cameraList=cameraList, camera=camera)
...
def change_mode():
    ...
    limit = session.get('limit')
    camera_list = Camera.query.all()
    session['all_camera'] = not all_camera
    print('Mode All Camera :', str(not all_camera))
    if not all_camera :
        # remake the thread
        for i in camera_list:
            threading_list[i.id] = cctvThread(i.id, limit)
        run_all_thread()
    else :
        stop_all_thread()
    return redirect(url_for('analytics', id=1))
...
def addCamera():
    ...
    cameraUrl=request.form['cameraUrl']
    limit = session.get('limit')
    all_camera = session.get('all_camera')
    new_data = Camera(cameraName=cameraName, cameraUrl=cameraUrl, createdAt=func.now())
    db.session.add(new_data)
    db.session.commit()
    db.session.refresh(new_data)
    threading_list[int(new_data.id)] = cctvThread(new_data.id, limit)
    if all_camera :
        print(new_data.id)
        threading_list[int(new_data.id)].start()
    return redirect(url_for('camera'))

@app.route('/edit_camera/<id>', methods=['POST'])
@login_req
def editCamera(id):
    limit = session.get('limit')
    all_camera = session.get('all_camera')
    ...
    Camera.query.filter_by(id=id).update(new_data)
    db.session.commit()
    if all_camera :
        threading_list[int(id)].stop()
        threading_list[int(id)] = cctvThread(id, limit)
        threading_list[int(id)].start()
    return redirect(url_for('camera'))

@app.route('/delete_camera/<id>')
@login_req
def deleteCamera(id):
    PolygonCoordinates.query.filter_by(cameraId=id).delete()
    Camera.query.filter_by(id=id).delete()
    threading_list[int(id)].stop()
    threading_list.pop(int(id))
    db.session.commit()
    return redirect(url_for('camera'))
...
@login_req
def settings(id):
    session['camera'] = Camera.query.filter_by(id=id).first().id
    settings_coordinates=getCoordinates(id)
    nama = session.get('username')
    all_camera = session.get('all_camera')
    cameraList = Camera.query.all()
    # will render the index.html file present in templates folder
    return render_template('settings.html', data=settings_coordinates, nama=nama, cameraList=cameraList, all_camera=all_camera)
...
def analytics(id):
    ...
    session['camera'] = Camera.query.filter_by(id=id).first().id
    cameraList = Camera.query.all()
    return render_template('analytics.html', jumlah=str(len(people_list)), masuk=str(len(enter_list)), keluar=str(len(out_list)), nama=nama, cameraList=cameraList, all_camera=all_camera)
...
def video_feed():
    cam = session.get('camera')
    limit = session.get('limit')
    return Response(annotatedStream(cam, limit), mimetype='multipart/x-mixed-replace; boundary=frame')

# =============================Threading fix (DON'T DELETE)==========================================

# @app.route('/background_feed')
# @login_req
# def background_feed():
#     return Response(cctvBackground.get_frame(), mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/background_feed')
@login_req
def background_feed():
    cam = session.get('camera')
    return Response(threading_list[cam].get_frame(), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def annotatedStream2():
    ...
    while True:
        ...
        if failed == True :
            ...
            print('enter count : '+str(len(enter_list)))
            print('out count : '+str(len(out_list)))
            for result in results:
                detections = []
                for r in result.boxes.data.tolist():
                    x1, y1, x2, y2, score, class_id = r
                    ...
                    class_id = int(class_id)
                    detections.append([x1, y1, x2, y2, score])
                tracker2.update(frame, detections)
...
def testing():
    ...
    # streams = streamlink.streams('https://www.youtube.com/watch?v=DjdUEyjx8GM&pp=ygURbGl2ZSBqYXBhbiBzdHJlZXQ%3D')
    # cap1 = cv2.VideoCapture(streams["480p"].url)
    # streams = streamlink.streams('https://www.youtube.com/watch?v=ybWlQJfQrwM')
    # cap2 = cv2.VideoCapture(streams["480p"].url)

    cap1 = cv2.VideoCapture('video/testing1.mp4') 
    cap2 = cv2.VideoCapture('video/testing3.mp4')
    ...
    while True:
        ...
        # success, frame = cap.read()
        failed = False
        success, frame1, failed = timerImage(cap1)
        success, frame2, failed = timerImage(cap2)
        if success == False:
            print('entered 2')
            success, frame1, failed = timerImage(cap1)
            success, frame2, failed = timerImage(cap2)
            frame1=cv2.resize(frame1,(880,440))
            frame2=cv2.resize(frame2,(880,440))
            # buffer=cv2.imencode('.jpg',frame)[1] # change to index 1 to get the buffer
            # frame=buffer.tobytes()# converting the image to bytes
            # yield(b'--frame\r\n' # yielding the frame for display
            #       b'Content-Type: image/jpeg\r\n\r\n'+frame+b'\r\n')
        ...
        if failed == True :
            ...
            reloaded+=1
            # cap = cv2.VideoCapture("rtsp://admin:admin123@sg2.labkom.us:5465/Streaming/Channels/201") #indoor office cctv
            cap1 = cv2.VideoCapture('video/testing1') 
            cap2 = cv2.VideoCapture('video/testing2') 
            # cap = cv2.VideoCapture(0)
            continue
            # break
        else:
            ...
            frame1=cv2.resize(frame1,(880,440))
            frame2=cv2.resize(frame2,(880,440))
            results1 = model.predict(frame1, classes=0)
            results2 = model.predict(frame2, classes=0)
            print('enter count : '+str(len(enter_list)))
            print('out count : '+str(len(out_list)))
            results1_dict = {
                'results' : results1,
            ...
            results_list = [results1_dict, results2_dict]
            for results in results_list:
                for result in results['results']:
                    ...
                    for r in result.boxes.data.tolist():
                        ...
                        class_id = int(class_id)
                        detections.append([x1, y1, x2, y2, score])
                    results['track'].update(results['frame'], detections)
                    for track in results['track'].tracks:
                        bbox = track.bbox
                        x1, y1, x2, y2 = bbox
                        x1 = int(x1)
                        y1 = int(y1)
                        ...
                        # if dist == 1 and (track_id in out_list.keys() or track_id in enter_list.keys()):
                        #     people_list[track_id] = y2
                        cv2.putText(results['frame'], (str(track_id)),(x1,y1),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,0,255),2)
                        cv2.rectangle(results['frame'], (x1, y1), (x2, y2), (colors[track_id % len(colors)]), 3)
                # print('Enter : ', enter_list)
                # print('Out : ', out_list)
                # annotate the frame using polygon
                
                results['frame']=cv2.polylines(results['frame'],[np.array(polygon_zone,np.int32)],True,(0,0,0),4,cv2.LINE_AA)
                cv2.line(results['frame'],(poly_zone['x1'], poly_zone['y1']),(poly_zone['x2'], poly_zone['y2']),(0,255,0),2)
                


                cv2.line(results['frame'],(poly_zone['x3'], poly_zone['y3']),(poly_zone['x4'], poly_zone['y4']),(0,0,255),2)
                cv2.rectangle(results['frame'], (0, 40), (200, 130), (255,255,255), -1)
                cv2.putText(results['frame'], ("Melewati Zona :"+str(len(people_list))),(10,60),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                cv2.putText(results['frame'], ("Masuk :"+str(len(enter_list))),(10,80),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                cv2.putText(results['frame'], ("Keluar :"+str(len(out_list))),(10,100),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
                cv2.putText(results['frame'], ("Kepadatan :"+str(len(enter_list)-len(out_list))),(10,120),cv2.FONT_HERSHEY_DUPLEX,0.5,(0,0,0),2)
            frame = np.hstack((results1_dict['frame'], results2_dict['frame']))
            buffer=cv2.imencode('.jpg',frame)[1] # change to index 1 to get the buffer
            frame=buffer.tobytes()# converting the image to bytes
            yield(b'--frame\r\n' # yielding the frame for display
                  b'Content-Type: image/jpeg\r\n\r\n'+frame+b'\r\n')
...
@app.route('/video_feed_testing')
@login_req
def video_feed_testing():
    return Response(testing(), mimetype='multipart/x-mixed-replace; boundary=frame')
...
# https://www.youtube.com/watch?v=DjdUEyjx8GM&pp=ygURbGl2ZSBqYXBhbiBzdHJlZXQ%3D
# https://www.youtube.com/watch?v=ybWlQJfQrwM  